function [neuron, chanvalue, channame]=rand_Parameter_Const(neuron_orig,neuron_orig2,boarder,num_sim,chan_del,ChangeInit)
...
for counter=1:num_sim
    ...
    for f=1:size(channame,1)    
        neuron{counter}.mech{1}.(channame{f,2}).(channame{f,1}).(channame{f,3})...
            =  neuron{counter}.mech{1}.(channame{f,2}).(channame{f,1}).(channame{f,3}) + (neuron_orig2.mech{1}.(channame{f,2}).(channame{f,1}).(channame{f,3})*(rand*diff(boarder)+boarder(1)));
    end
