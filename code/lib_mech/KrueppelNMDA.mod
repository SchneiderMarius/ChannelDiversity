...
synaptic current with exponential rise and decay conductance defined by
        ...
        where
         g = 0 for t < onset and
         g=mgblock * amp * ((1-exp(-(t-onset)/tau0))-(1-exp(-(t-onset)/tau1)))
          for t > onset
...
NEURON {
	POINT_PROCESS KrueppelNMDA
	RANGE R, R0, onset, tau1, tau2, eta, gamma, shift, mg, gmax, e, Fire
...
PROCEDURE release() {
	...
	if ( lastrelease > 0 ) {
		R =  exptable(-(t-lastrelease)/tau2) - exptable(-(t-lastrelease)/tau1) + R0 * exptable(-(t-lastrelease)/tau1) :tau1 and 2 was changed since tau1 being rise time is more logic
	}
