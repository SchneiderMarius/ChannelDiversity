function [Rin, tau, cap, Vrest, v0, vs,Spike] = t2n_passTests_marius(neuron,tree,targetfolder_results,ostruct)
...
switch ostruct.passtest
     case 'Lin'
         ...
         if ~isfield(ostruct,'Vh')
            Vh = -70  ;   
         else
            Vh=ostruct.Vh;
         end          

        % -50pA Meranfahrd 2015
        [hstep, Vrest] = t2n_findCurr(neuron,tree,-70);
%         [hstep, Vrest] = t2n_findCurr(neuron,tree,-73);
    ...
    case 'Perinia'
        ...
        if ~isfield(ostruct,'amp')
          amp = -0.05  ;   
            ...
            % 10pA for tau...Mongiat 2009
         [hstep, Vrest] = t2n_findCurr(neuron,tree,-80);  % LJP corrected
        for t = 1:numel(tree)
%             neuron.pp{t}.IClamp = struct('node',stimnode(t),'times',[-400,del,del+dur],'amp', [0, amp 0]); %n,del,dur,amp
...
%         amp = -0.01  ;      % 10pA for tau...Mongiat 2009
        [hstep, Vrest] = t2n_findCurr(neuron,tree,-85+15);  % LJP corrected
        hstep=amp
        for t = 1:numel(tree)
            neuron.pp{t}.IClamp = struct('node',stimnode(t),'times',[-400,del,del+dur],'amp', [0, hstep(t) 0]); %n,del,dur,amp
            neuron.record{t}.cell = struct('record','v','node',recordnode(t));
        end
...
%         neuron.params.cvode = 1;
        if ~isfield(ostruct,'amp')
            ...
            %mV for cap and Rin Mongiat 2009
        for t = 1:numel(tree)
...
case {'Lin'}
        for t = 1:numel(tree)
            ...
            v0(t) = V0;
            Spike(t)=numel(out.APCtimes{t}{1});
    ...
    case {'SH','Mongiat2','Mehranfard','Std','Perinia'}
        for t = 1:numel(tree)
            V0 = mean(out.record{t}.cell.v{recordnode(t)}(1:del/neuron.params.dt+1));  %mV  only works with prerun
            v0(t) = V0;
