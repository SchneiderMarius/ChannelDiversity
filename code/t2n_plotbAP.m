...
%             latency = (bAP{t}(:,2+5)-bAP{t}(som,2+5));
    ...
    if (nargout == 0 || nargout > 4) && exist('targetfolder_results','var') && ~isempty(targetfolder_results)
        ...
        if ostruct.relamp
            ylim([0 1])
            tprint(fullfile(targetfolder_results,sprintf('Fig6_bAP-rel-ampl_%s',neuron{n}.experiment)),'-jpg')
        else
            ylim([0 150])
            tprint(fullfile(targetfolder_results,sprintf('Fig6_bAP-ampl_%s',neuron{n}.experiment)),'-jpg')
        end
        figure(fig(2))
        if ostruct.plotData
            if ostruct.relamp
        ...
        tprint(fullfile(targetfolder_results,sprintf('Fig6_bAP-del_%s',neuron{n}.experiment)),'-jpg')
        figure(fig(n+2))
        ylim(ylims)
        xlim(xlims)
        ostruct.image = 1 ;
        ...
        set(c,'YTick',[-80,0,80])
        tprint(t2n_catName(targetfolder_results,'Fig6_bAP-trees',neuron{n}.experiment),'-SHR-tif')
    end
