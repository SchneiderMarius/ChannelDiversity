function [neuron, chanvalue, channame]=randomize_Conductance2(neuron_orig,boarder,num_sim,chan_del)
...
for counter=1:num_sim
    ...
    for f=1:size(channame,1)    
        neuron{counter}.mech{1}.(channame{f,2}).(channame{f,1}).(channame{f,3})...
            =  neuron{counter}.mech{1}.(channame{f,2}).(channame{f,1}).(channame{f,3})*rand*boarder(f);
    end
