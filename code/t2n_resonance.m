function [imp,freq] = t2n_resonance(neuron,tree,amp,holding_voltage,targetfolder_data,doerrorbar)
...
% *****************************************************************************************************
if (nargin < 5) || isempty (targetfolder_data)
    doerrorbar = 0;
end
 if (nargin < 4) || isempty (holding_voltage)
    holding_voltage = -70;
 end
if (nargin < 6)
    doerrorbar = 0;
...
ylabel('Impedance [M\Omega]')
box off
save(fullfile(sprintf('Exp_resonance_%s.mat',neuron.experiment)),'locs','locs2','imp','pks','pks2','pks2')
