...
if (nargin < 1) || isempty (kainfo)
    warning      (['You need to define the information for the' ....
        'A-type K+ conductance disribution']);
end
if numel(kainfo.region) == 1
    isregion = ...
        (find (strcmp (tree.rnames, kainfo.region)));
    isregion = ...
        (tree.R == isregion);
elseif numel (kainfo.region) > 1
    ...
    warning      (['Define the regions where the A-type K+ conductance' ...
        'will be distributed as a function of distance from the soma']);
end
...
isregion_100                       = ...   % region within first 100 um
    (tree.R == find (strcmp (tree.rnames, 'proxAp')));  
isregion_300                       = ...   % region within first 300 um
    (tree.R == find (strcmp (tree.rnames, 'middleAp')));  
isregion_500                       = ...   % region within first 500 um
    (tree.R == find (strcmp (tree.rnames, 'distalAp')));  
isregion_tuft                      = ...
